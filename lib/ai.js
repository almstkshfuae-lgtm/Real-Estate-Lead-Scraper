 ...
 */
async function withRetry(fn, maxAttempts = 8, baseDelayMs = 3000) {
...
export const GLOBAL_AREAS_COORDS = {
    // Saudi Arabia
 ...
 */
function verifyLeadInSource(name, nameAr, sourceText, company, companyAr) {
    if (!name || !sourceText)
        return false;
...
export async function generateGeminiChatStream(systemPrompt, messages, maxTokens = 2048, signal) {
    var _a;
    const config = await getAIConfig();
    if (!config) {
        throw new Error("No AI provider configured. Set GOOGLE_AI_API_KEY.");
...
export async function extractProjectData(text) {
    const content = await generateGeminiText(`You are an expert Real Estate Data Extractor. 
    ...
    Return a JSON array of objects.`, `Extract projects from this text: ${text}`, 2048);
