...
export async function getAIConfig() {
    ...
    const envGoogleKey = (_a = (getEnvVar('GOOGLE_AI_API_KEY') || getEnvVar('GOOGLE_API_KEY'))) === null || _a === void 0 ? void 0 : _a.trim();
    const rawGoogleKey = envGoogleKey
        ? envGoogleKey
        : (_b = (await getSecret("googleAiApiKey"))) === null || _b === void 0 ? void 0 : _b.trim();
    const googleApiKey = rawGoogleKey && !rawGoogleKey.startsWith('YOUR_') ? rawGoogleKey : null;
    const googleProjectId = (_c = (getEnvVar('GOOGLE_AI_PROJECT_ID') || getEnvVar('GOOGLE_PROJECT_ID') || getEnvVar('GOOGLE_CLOUD_PROJECT') || getEnvVar('GCLOUD_PROJECT'))) === null || _c === void 0 ? void 0 : _c.trim();
    const googleLocation = (_d = (getEnvVar('GOOGLE_AI_LOCATION') || "us-central1")) === null || _d === void 0 ? void 0 : _d.trim();
    let googleModel = (_e = (getEnvVar('GOOGLE_AI_MODEL') || getEnvVar('GOOGLE_MODEL') || "gemini-2.5-flash")) === null || _e === void 0 ? void 0 : _e.trim();
...
function formatCriteriaPrompt(criteria) {
    ...
    if (criteria.budgetMin !== undefined || criteria.budgetMax !== undefined) {
        lines.push(`Budget minimum: ${(_a = criteria.budgetMin) !== null && _a !== void 0 ? _a : "any"}`);
        lines.push(`Budget maximum: ${(_b = criteria.budgetMax) !== null && _b !== void 0 ? _b : "any"}`);
    }
...
function filterLeadByCriteria(lead, criteria) {
    var _a, _b;
    if (!criteria || typeof criteria !== "object") {
        return true;
    }
    if (Array.isArray(criteria.emirates) && criteria.emirates.length > 0) {
        ...
        const matchesLocation = criteria.emirates.some((emirate) => leadLocation.includes(emirate.toLowerCase()));
        if (!matchesLocation) {
            return false;
        }
    }
    ...
    if (Array.isArray(criteria.signals) && criteria.signals.length > 0) {
        const leadSignals = Array.isArray(lead.signals) ? lead.signals.map((s) => String(s).toLowerCase()) : [];
        const matchesSignal = criteria.signals.some((signal) => leadSignals.some((ls) => ls.includes(signal.toLowerCase())) ||
    ...
    if (criteria.budgetMin !== undefined || criteria.budgetMax !== undefined) {
        const minBudget = Number(((_a = lead.budgetMin) !== null && _a !== void 0 ? _a : lead.budget) || 0);
        const maxBudget = Number(((_b = lead.budgetMax) !== null && _b !== void 0 ? _b : lead.budget) || 0);
...
function extractTextFromAIResponse(response) {
    ...
    const candidate = ((_a = response === null || response === void 0 ? void 0 : response.predictions) === null || _a === void 0 ? void 0 : _a[0]) || ((_b = response === null || response === void 0 ? void 0 : response.candidates) === null || _b === void 0 ? void 0 : _b[0]) || ((_c = response === null || response === void 0 ? void 0 : response.choices) === null || _c === void 0 ? void 0 : _c[0]) || ((_d = response === null || response === void 0 ? void 0 : response.output) === null || _d === void 0 ? void 0 : _d[0]) || (response === null || response === void 0 ? void 0 : response.output) || response;
    let contents = ((_e = candidate === null || candidate === void 0 ? void 0 : candidate.message) === null || _e === void 0 ? void 0 : _e.content) || (candidate === null || candidate === void 0 ? void 0 : candidate.content) || (candidate === null || candidate === void 0 ? void 0 : candidate.output) || (candidate === null || candidate === void 0 ? void 0 : candidate.text) || candidate;
    if (!contents) {
        return "";
    }
    ...
    if (Array.isArray(contents)) {
        return contents
            .map((item) => (typeof item === "string" ? item : (item === null || item === void 0 ? void 0 : item.text) || ""))
            .filter(Boolean)
            .join("\n");
    }
...
async function withRetry(fn, maxAttempts = 3, baseDelayMs = 1000) {
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            console.warn(`[AI] Gemini rate-limited (attempt ${attempt}/${maxAttempts}). Retrying in ${Math.round(delay)}ms...`);
...
export function resolveCoords(location) {
    const normalized = (location === null || location === void 0 ? void 0 : location.trim()) || "";
...
export function normalizeLocation(loc) {
    ...
    if (lower.includes("ياس") || lower.includes("yas"))
        return "Yas Island";
    if (lower.includes("ريم") || lower.includes("reem"))
        return "Al Reem Island";
    if (lower.includes("سعديات") || lower.includes("saadiyat"))
        return "Saadiyat Island";
    if (lower.includes("خالدية") || lower.includes("khalidiyah") || lower.includes("khalidiya"))
        return "Khalidiyah";
    if (lower.includes("راحه") || lower.includes("raha"))
        return "Al Raha Beach";
    if (lower.includes("كورنيش") || lower.includes("corniche"))
        return "Corniche";
    if (lower.includes("مارينا") || lower.includes("marina"))
        return "Dubai Marina";
    if (lower.includes("نخلة") || lower.includes("palm"))
        return "Palm Jumeirah";
    if (lower.includes("وسط المدينة") || lower.includes("downtown"))
        return "Downtown Dubai";
    if (lower.includes("خليج الأعمال") || lower.includes("business bay"))
        return "Business Bay";
    if (lower.includes("جميرا") || lower.includes("jumeirah"))
        return "Jumeirah";
    if (lower.includes("العالمي") || lower.includes("difc"))
        return "DIFC";
    if (lower.includes("ممشى جي بي آر") || lower.includes("jbr"))
        return "JBR";
    if (lower.includes("المرابع") || lower.includes("ranches"))
        return "Arabian Ranches";
    if (lower.includes("البرشاء") || lower.includes("barsha"))
        return "Al Barsha";
    if (lower.includes("مردف") || lower.includes("mirdif"))
        return "Mirdif";
    if (lower.includes("ديرة") || lower.includes("deira"))
        return "Deira";
    if (lower.includes("بر دبي") || lower.includes("bur dubai"))
        return "Bur Dubai";
    if (lower.includes("قرية جميرا") || lower.includes("jvc"))
        return "JVC";
    if (lower.includes("شارقة") || lower.includes("sharjah"))
        return "Sharjah City";
    if (lower.includes("نهدة") || lower.includes("nahda"))
        return "Al Nahda";
    if (lower.includes("خان") || lower.includes("khan"))
        return "Al Khan";
    if (lower.includes("عجمان") || lower.includes("ajman"))
        return "Ajman";
    if (lower.includes("خيمة") || lower.includes("khaimah") || lower.includes("rak"))
        return "Ras Al Khaimah";
    if (lower.includes("فجيرة") || lower.includes("fujairah"))
        return "Fujairah";
    if (lower.includes("دبي") || lower.includes("dubai"))
        return "Dubai";
    if (lower.includes("أبوظبي") || lower.includes("abu dhabi") || lower.includes("abu_dhabi"))
        return "Abu Dhabi";
    if (lower.includes("أم القيوين") || lower.includes("quwain") || lower.includes("uaq"))
...
export function parseBudgetToFloat(val) {
...
function extractLikelyNames(content) {
...
function extractLikelyRole(content) {
    const patterns = [
...
function heuristicExtractLeads(scrapedData, criteria) {
    var _a, _b;
    ...
    const location = ((_a = scrapedData.type) === null || _a === void 0 ? void 0 : _a.includes("Dubai")) ? "Dubai" : ((_b = scrapedData.type) === null || _b === void 0 ? void 0 : _b.includes("Abu Dhabi")) ? "Abu Dhabi" : "Abu Dhabi";
...
async function generateGeminiText(systemPrompt, userPrompt, maxTokens = 1024, signal) {
 ...
 */
export async function extractHNWILeads(scrapedData, criteria) {
    const criteriaPrompt = formatCriteriaPrompt(criteria);
    const cleanedContent = cleanScrapedText(scrapedData.content);
    const content = await generateGeminiText(`You are an expert at extracting high-net-worth individual (HNWI) leads from UAE business websites, club directories, news articles, and event listings.
...
Output ONLY the JSON array. No other text.`, `Extract leads from this content:\n\nPage Title: ${scrapedData.title}\nSource: ${scrapedData.name}\nType: ${scrapedData.type}\n\nContent:\n${cleanedContent}`, 4096);
    if (!content) {
    ...
    if ((!Array.isArray(leads) || leads.length === 0) && isLikelyTruncated(content)) {
        ...
        const retryContent = await generateGeminiText(`You are an expert at extracting HNWI leads from UAE business content.
...
Output ONLY the JSON array. No other text.`, `Page Title: ${scrapedData.title}\nSource: ${scrapedData.name}\nContent (truncated):\n${cleanedContent.substring(0, 6000)}`, 4096);
        if (retryContent) {
    ...
    return Array.isArray(leads)
        ? leads
            .filter((lead) => lead.name && lead.company && lead.role && lead.tier && lead.score !== undefined && lead.location
 ...
 */
function verifyLeadInSource(name, nameAr, sourceText) {
    if (!name || !sourceText)
        return false;
 ...
 */
export async function extractLeadsFromText(text, criteria) {
    const criteriaPrompt = formatCriteriaPrompt(criteria);
    const cleanedText = cleanScrapedText(text);
    const content = await generateGeminiText(`You are an expert at extracting high-quality business leads from UAE news articles, event write-ups, and profile snippets.
    ...
    ${criteriaPrompt}`, `Extract leads from this text: ${cleanedText}`, 1024);
    if (!content) {
...
export async function enrichLeadWithAI(lead) {
    ...
    const enrichedLead = {
        ...
        budgetMin: parsedMin,
        budgetMax: parsedMax,
        relocated: (_a = lead.relocated) !== null && _a !== void 0 ? _a : null,
        propertyPref: lead.propertyPref || null
    };
    if (lead.tier && lead.score !== undefined) {
        return enrichedLead;
    }
    const content = await generateGeminiText(`You are a real estate investment analyst specializing in UAE luxury real estate leads.
...
Return ONLY this JSON object with keys tier and score. No explanatory text.`, JSON.stringify({
 ...
 */
export async function generatePersonaAnalysis(lead, lang = "en") {
    if (lead.persona) {
    ...
    const content = await generateGeminiText(`You are a professional behavioral psychologist and UAE real estate investment analyst.
...
Do not use placeholders. Use the data provided.`, JSON.stringify({
        name: lead.name,
 ...
 */
export async function enrichLeadsInBatch(leads) {
    return Promise.all(leads.map(lead => enrichLeadWithAI(lead)));
}
export async function generateGeminiStream(systemPrompt, userPrompt, maxTokens = 1024, signal) {
    var _a;
    const config = await getAIConfig();
    if (!config) {
        throw new Error("No AI provider configured. Set GOOGLE_AI_API_KEY.");
        ...
        async pull(controller) {
            ...
            try {
                ...
                if (cleanChunk.startsWith(','))
                    cleanChunk = cleanChunk.substring(1).trim();
                if (cleanChunk.startsWith('['))
                    cleanChunk = cleanChunk.substring(1).trim();
                if (cleanChunk) {
                    try {
                        const obj = JSON.parse(cleanChunk);
                        const candidate = ((_a = obj === null || obj === void 0 ? void 0 : obj.predictions) === null || _a === void 0 ? void 0 : _a[0]) || ((_b = obj === null || obj === void 0 ? void 0 : obj.candidates) === null || _b === void 0 ? void 0 : _b[0]);
                        const contents = (candidate === null || candidate === void 0 ? void 0 : candidate.content) || candidate;
                        const parts = (contents === null || contents === void 0 ? void 0 : contents.parts) || contents;
                        ...
                        else if (parts === null || parts === void 0 ? void 0 : parts.text) {
                            text = parts.text;
                        }
                    }
                    catch (e) {
