...
class ScraperClient {
     ...
     */
    async scrapeMultipleSources(sourceKeys, webhookUrl, runId, criteria) {
     ...
     */
    async scrapeSourceSync(sourceKey) {
 ...
 */
ScraperClient.DEFAULT_HANDSHAKE_TIMEOUT_MS = 10000; // 10s — health checks
ScraperClient.DEFAULT_SCRAPE_TRIGGER_TIMEOUT_MS = 15000; // 15s — scraper responds immediately with queued ack
// Singleton instance promise
let scraperClientPromise = null;
...
export function getScraperClient() {
...
export function getWebhookUrl(requestOrigin) {
