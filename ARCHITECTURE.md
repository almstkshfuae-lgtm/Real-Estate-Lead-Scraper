...
- **Configurable Zombie Watchdog**: Employs an asynchronous timer (configurable via `SCRAPER_ZOMBIE_KILL_MS` with a 25-minute default) that tracks crawl diagnostics in a `jobDiagnostics` state object. If the job hangs and exceeds the timeout limit, the watchdog automatically logs the diagnostics, dispatches a failure webhook containing the last visited URL and pages scraped to Next.js, and calls `browserInstance.close()` to immediately terminate the Playwright Chromium processes and reclaim RAM.
