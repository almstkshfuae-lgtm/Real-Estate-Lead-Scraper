...
- **Robust Watchdog & Lifecycle Management**: 
  - **Asynchronous Safe Close**: All `browser.close()` calls are protected by a non-blocking `Promise.race` wrapper with a 5-second timeout, ensuring that hung Playwright browser processes never stall queue execution. If the close operation times out or fails, the service retrieves the browser's OS process via `browser.process()` and issues a `SIGKILL` signal to immediately terminate the process tree at the operating system level, preventing memory leaks and zombie processes entirely.
...
- **Concurrent In-Memory Queue**: Rather than rejecting overlapping scrape requests with a `429` status code, the service queues incoming scraper requests in a FIFO `scrapeQueue`. The active jobs are executed concurrently (`MAX_CONCURRENT_SCRAPES = 2` by default, configurable) to enable faster parallel processing while bounding RAM usage. To prevent duplicate executions and API token waste in distributed or server restart scenarios, the `ScrapeQueueManager` performs an immediate database status lookup inside `enqueueJob`. If the target `ScrapeRun` is already in a `PROCESSING`, `COMPLETED`, or `FAILED` state, it deduplicates the request and returns a success response with status `already_queued`. In addition, before executing any job from the queue, `processQueue` re-verifies the DB status, skipping any jobs that have been cancelled or marked as failed in the database while waiting in the queue. Newly created scraper runs are saved with a `"PENDING"` status, transitioning to `"PROCESSING"` when execution starts (triggered via an `isStartedSignal` webhook sent to the Next.js app). Exposes `GET /queue` to allow operators to inspect the execution pipeline diagnostics in real-time.
...
- **AI Gateway Reasoning Optimization**: To prevent JSON output truncation and reduce latency/token consumption, the centralized AI gateway (`lib/ai-gateway.ts`) automatically configures `thinkingConfig: { thinkingBudget: 0 }` for all Gemini 2.x models. This disables reasoning/thinking tokens by default, ensuring immediate, fast, and structured JSON output within tight output token limits.
- **Null Safety in Logs**: The AI gateway wraps error-logging blocks with null-safe accessors (`config?.model || "unknown"`) to prevent TypeErrors from masking the underlying connection or quota failures.
