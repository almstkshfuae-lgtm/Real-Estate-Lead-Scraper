import { shouldRefreshAndCreateNewToken, createToken } from '../lib/auth';
import { getScraperClient, ScraperClient } from '../lib/scraper-client';
...
async function runTests() {
  ...
  const nearExpiryToken = jwt.sign(
    { ...testUser, exp: Math.floor(Date.now() / 1000) + 24 * 60 * 60 },
    secret
  );
  ...
  const decodedNew: any = jwt.verify(refreshedToken, secret);
  ...
  const freshToken = jwt.sign(
    { ...testUser, exp: Math.floor(Date.now() / 1000) + 6 * 24 * 60 * 60 },
    secret
  );
