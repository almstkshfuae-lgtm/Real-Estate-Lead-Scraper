...
async function runFiltersTest() {
    ...
    const allRelocated = dataRelocated.leads.every((l) => l.relocated === true);
    if (allRelocated) {
    ...
    const allNoRental = dataExcludeRental.leads.every((l) => l.rentalFlag === false);
    if (allNoRental) {
    ...
    const allValidTier = dataTierMin.leads.every((l) => l.tier <= 2);
    if (allValidTier) {
