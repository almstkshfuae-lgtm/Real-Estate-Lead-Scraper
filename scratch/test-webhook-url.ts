...
function runTests() {
  ...
  for (const tc of testCases) {
    ...
    // Apply test env
    if (tc.env.WEBHOOK_URL !== undefined) process.env.WEBHOOK_URL = tc.env.WEBHOOK_URL;
    else delete process.env.WEBHOOK_URL;

    if (tc.env.APP_URL !== undefined) process.env.APP_URL = tc.env.APP_URL;
    else delete process.env.APP_URL;

    if (tc.env.NEXT_PUBLIC_APP_URL !== undefined) process.env.NEXT_PUBLIC_APP_URL = tc.env.NEXT_PUBLIC_APP_URL;
    ...
    // Restore env
    if (originalWebhookUrl !== undefined) process.env.WEBHOOK_URL = originalWebhookUrl;
    else delete process.env.WEBHOOK_URL;

    if (originalAppUrl !== undefined) process.env.APP_URL = originalAppUrl;
    else delete process.env.APP_URL;

    if (originalNextPublicAppUrl !== undefined) process.env.NEXT_PUBLIC_APP_URL = originalNextPublicAppUrl;
    else delete process.env.NEXT_PUBLIC_APP_URL;
  }
