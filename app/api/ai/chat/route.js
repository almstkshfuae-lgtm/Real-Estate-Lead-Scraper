import { getAIConfig, generateGeminiChatStream } from "@/lib/ai";
import { getSession } from "@/lib/auth";
import { prisma } from "@/lib/prisma";
const buildSystemPrompt = (lang, context, user) => {
    const profileHints = [];
    if (user) {
...
export async function GET() {
    ...
    return new Response(JSON.stringify(history.map((message) => ({ role: message.role, content: message.content }))), {
        headers: { "Content-Type": "application/json" },
    });
}
    ...
    try {
        ...
        if (lastUserMessage === null || lastUserMessage === void 0 ? void 0 : lastUserMessage.content) {
            try {
                ...
                console.error('[AI Chat] failed to save user message', dbErr.message);
            }
        }
        let accumulatedText = "";
                    ...
                    if (done) {
                        ...
                        if (accumulatedText.trim()) {
                            try {
                                ...
                                console.error('[AI Chat] failed to save assistant message to DB', dbErr.message);
                            }
                        }
        ...
        return new Response(JSON.stringify({ error: "Failed to process chat", detail: error === null || error === void 0 ? void 0 : error.message }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
}
