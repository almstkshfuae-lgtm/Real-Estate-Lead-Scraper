...
- [x] **Security Audit: AI JSON, SSE Cleanup & Schema Strict Mode** — Confirmed `api/ai/signals` already uses `parseAIJson` + try/catch (no raw JSON.parse). Confirmed `api/scrape-runs/[id]/sse` already registers `request.signal.addEventListener('abort', cleanup)` to stop polling on client disconnect. Added `.strict()` to `leadUpdateSchema` in `lib/schemas.ts` so requests containing system-reserved fields (`id`, `agentId`, `createdAt`, `updatedAt`, `deletedAt`) are actively rejected with HTTP 400 instead of being silently stripped. <!-- id: 12.19 -->
- [x] **Hardened: Multi-Language UI Parity & Localization Fallbacks** — Resolved the gap where toast errors, alerts, and field placeholders in LeadSidebar, LeadTable, CsvUpload, NotificationListener, and ProjectSidebar were hardcoded. Added missing keys to both locales, ensuring a fully unified Arabic and English experience without hardcoded English defaults. <!-- id: 12.20 -->
- [x] **Hardened: Prevent Empty-String Database Crashes on Optional Numeric Fields** — Sanitized and robustly parsed empty or whitespace budget fields to null on both frontend and backend update endpoints, and added unit tests. <!-- id: 12.21 -->
- [x] **Hardened: Lead Cluster Map Stacking Bug** — Replaced `Math.random()` jitter (which only applied to fallback coords) with a stable deterministic ID-based hash jitter applied to ALL leads. Fixes 465 leads sharing identical DB coordinates (e.g. all "Abu Dhabi" leads from scraper backfill) rendering as a single invisible stack instead of a visible spread. <!-- id: 12.22 -->\n- [x] **Hardened: User-Facing Error Message Sanitization** — Replaced all technical strings exposed to the frontend: (1) `api/ai/chat` budget-exceeded and missing-API-key errors now emit structured error codes (`daily_budget_exceeded`, `ai_unavailable`) with server-side logging of raw figures; (2) `lib/safe-fetch.ts` `safeJson` logs the raw non-JSON/HTML proxy body server-side and throws a generic `server_connection_error` token; (3) `scraper-service/src/ui-strings.js` gained a `USER_FACING_MESSAGES` export with EN/AR plain-language strings for consent bypass failures, bot blocks, and selector failures — preventing any CSS selector string from leaking into notifications. New i18n keys added to both `en/common.json` and `ar/common.json`. <!-- id: 12.23 -->
- [/] **Hardened: Geofencing Text Fallback Full-Text Search (FTS) Indexing** — Migrate location search to use MySQL full-text search indexes instead of multi-contains queries. <!-- id: 12.24 -->
